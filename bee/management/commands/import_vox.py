...
from bee.models import Post, Identity, TrustGroup
...
def main(argv=None):
    ...
    logging.getLogger('django.db').setLevel(level=logging.ERROR)
    ...
    try:
        ident = Identity.objects.get(identifier=openid)
    except Identity.DoesNotExist:
        logging.info('Creating new identity mapping to %s for %s', person.username, openid)
        ident = Identity(identifier=openid, user=person)
        ident.save()
    else:
...
def person_for_openid(openid, display_name):
    ident_obj, created = Identity.objects.get_or_create(identifier=openid)
    return ident_obj
...
def import_assets(source):
    ...
    for asset_el in tree.findall('{http://www.w3.org/2005/Atom}entry'):
        ...
        private_group = TrustGroup.objects.get(tag='private')
        for privacy in privacies:
            ...
            try:
                ...
                group, created = TrustGroup.objects.get_or_create(tag=group_ref,
    ...
    for comment_el in tree.findall('{http://www.w3.org/2005/Atom}entry'):
        ...
        try:
            parent_asset = Post.objects.get(atom_id=parent_ref)
        except Post.DoesNotExist:
            # OOPS
